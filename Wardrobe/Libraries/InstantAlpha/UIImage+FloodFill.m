...
-(UIImage *)changeWhiteColorTransparent: (UIImage *)image
{
    const CGFloat colorMasking[6]={125.0, 200.0, 125.0, 200.0, 125.0, 200.0};
...
{
    return [self changeWhiteColorTransparent: [self floodFillFromPoint:startPoint withColor:newColor andTolerance:tolerance useAntiAlias:YES withFrame:nil]];
}

- (UIImage *) floodFillFromPoint:(CGPoint)startPoint withColor:(UIColor *)newColor andTolerance:(int)tolerance
withFrame:(UIImageView *)frame
{
    UIImage *image = [self floodFillFromPoint:startPoint withColor:newColor andTolerance:tolerance useAntiAlias:YES withFrame:frame];
    return image;
}
...
- (UIImage *) floodFillFromPoint:(CGPoint)startPoint withColor:(UIColor *)newColor andTolerance:(int)tolerance useAntiAlias:(BOOL)antiAlias withFrame:(UIImageView *)imframe
{
    @try
    {
        ...
        */
        ...
        FloodFillImageView *fl=  (FloodFillImageView*)imframe;
        
        
        CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
        
        CGImageRef imageRef = [fl.chImageView.image CGImage];
        ...
        float xscale = width / imframe.frame.size.width;
        float yscale = height / imframe.frame.size.height;
        
        startPoint.x = startPoint.x * xscale;
        startPoint.y = startPoint.y * yscale;
        
        CGFloat xx = 1.0;
        if ([self respondsToSelector:@selector(scale)]) xx = self.scale;
        
      unsigned char *imageData =  malloc((height*yscale) * (xscale* width) * 4 );
